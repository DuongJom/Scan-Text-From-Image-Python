...
class App():
    ...
    def extract_text(self):
        if self.cbbPath.get():
            ...
            self.columns = p_table.field_names = ["Số hiệu", "Ngày/tháng/năm"]
            p_table.align['Số hiệu'] = 'l'
            ...
            for line in content:
                ...
                if so_hieu != '' and ngay != '':
                    ...
                    self.data.append([so_hieu.split("số ")[1], ngay])
    ...
    def export_to_excel(self):
        ...
        file = fd.asksaveasfile(filetypes=filetypes, mode='a')
        if file:
            df = pd.DataFrame(self.data, columns=self.columns)
            path = file.name
            if os.path.splitext(path)[1] == '':
                path = path + ".xlsx"
            df.to_excel(os.path.abspath(path), engine='xlsxwriter')